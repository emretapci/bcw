...
const PreferencesScreen = () => {
	...
	useEffect(() => AsyncStorage.getItem('darkMode').then(darkMode => setDarkModeOn(darkMode ? JSON.parse(darkMode) : false)), []);
	useEffect(() => AsyncStorage.setItem('darkMode', JSON.stringify(darkModeOn)), [darkModeOn]);
			...
			<ScrollView>
				<List.Section>
					<List.Item
						...
						right={() => <Switch value={darkModeOn} onValueChange={() => setDarkModeOn(!darkModeOn)} />}
					/>
					<List.Item
						title='Currency'
